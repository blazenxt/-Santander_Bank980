# ╔══════════════════════════════════════════════════════════════╗
# ║     BlazeNXT Bot — Full System Bot                        ║
# ║     Numbers + OTP Forwarding + Full Admin Suite             ║
# ║     Developed by BlazeNXT (@firstoget)                     ║
# ╚══════════════════════════════════════════════════════════════╝
...
#  CONFIGURATION
# ═══════════════════════════════════════════════════════════════
BOT_TOKEN      = "8527443954:AAESFgu86ckaMX_-kjdEGnYdAgOOda07q2I"
OWNER_IDS      = [7064636445, 7476419498, 8057461514]
ADMIN_IDS      = [7064636445, 7476419498, 8057461514]
OTP_GROUP_LINK = "https://t.me/+DfUnv_qgZEFkZWU8"
BOT_NAME       = "BlazeNXT Bot"

REQUIRED_CHANNELS = [
    {"name": "BlazeNXT Backup",      "link": "https://t.me/blazenxt_backup",                                   "id": "@blazenxt_backup"},
    {"name": "BlazeNXT OTP",         "link": "https://t.me/+DfUnv_qgZEFkZWU8",                           "id": "@blazenxt_otp"},
    {"name": "BlazeNXT Numbers", "link": "https://t.me/blazenxt_numbers",                                 "id": "@blazenxt_numbers"},
]
...
def format_nxt_number(number: str) -> str:
    """
    Format number as: +XXX-NXT-XX
    Example: +923-NXT-55035
    Always ensures 3-digit prefix.
    """
    digits = re.sub(r"[^0-9]", "", number)
    if len(digits) >= 5:
        prefix = digits[:3].zfill(3)  # Ensure 3-digit prefix
        return f"+{prefix}-NXT-{digits[-5:]}"
    return digits
...
def format_otp_message(number: str, service: str, otp: str,
    ...
    # Number formatted as: +923-NXT-55035
    nxt_number = format_nxt_number(number)
    ...
    return f"{country_custom_emoji} ┃ {app_custom_emoji}  <b>#{region}</b>  <b>{nxt_number}</b>  <b>#{lang}</b>{gh_tag}"
...
def get_otp_keyboard(number: str, otp: str) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup([
        ...
        [
            ...
            InlineKeyboardButton(
                " WhatsApp",
                url="https://chat.whatsapp.com/HVqCQ6wX0sz9sqox5QEgrg?mode=hqctcla",
                api_kwargs={
                    "style": "primary",
...
async def send_otp_to_owner(number: str, service: str, otp: str,
    ...
    # Number in NXT format
    nxt_number = format_nxt_number(number)

    msg = f"{country_custom_emoji} ┃ {app_custom_emoji}  <b>#{region}</b>  <b>{nxt_number}</b>  <b>#{lang}</b>"
...
def _detect_api_type(cfg: dict) -> str:
    ...
    # ── 2. PS API probe (?key=) ───────────────────────────────
    try:
        base_url = cfg["url"].split("?")[0].rstrip("/")
        token    = cfg.get("token", "").strip()
        _ps_headers = {
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36",
            "Accept": "application/json, text/plain, */*",
            "Referer": "https://pscall.net/",
        }
        r = requests.get(f"{base_url}?key={token}", headers=_ps_headers, timeout=8)
...
def _fetch_ps_api(cfg: dict, limit: int) -> List[Dict]:
    ...
    Uses browser User-Agent + raw key URL to bypass server IP restrictions.
    """
    base_url = cfg["url"].split("?")[0].rstrip("/")
    token    = cfg.get("token", "").strip()
    ...
    raw = _safe_json(r)
    if raw is None:
        logger.warning(f"PS API [{base_url}?key=***] — empty/non-JSON response (HTTP {r.status_code}), skipping")
        return []
    logger.info(f"PS API [{base_url}] — got {len(raw) if isinstance(raw, list) else type(raw).__name__} records")
...
def bc(text, cb=None, url=None, style=None):
    """Button helper - style parameter removed (not supported by Telegram API)"""
    if cb:
...
def get_main_menu_keyboard():
    return InlineKeyboardMarkup([
        [
            bc("🧇 𝗚𝗲𝘁 𝗡𝘂𝗺𝗯𝗲𝗿",  cb="show_countries"),
            bc("🫁 𝗣𝗿𝗼𝗳𝗶𝗹𝗲",       cb="user_profile"),
        ],
        [
            bc("🧠 𝗗𝗲𝘃𝗲𝗹𝗼𝗽𝗲𝗿",     url="https://t.me/firstoget"),
        ],
    ])

def get_join_keyboard():
    return InlineKeyboardMarkup([
        [bc("📞 Nexa Numbers",    url="https://t.me/blazenxt_numbers")],
        [bc("💬 WhatsApp Group",  url="https://chat.whatsapp.com/HVqCQ6wX0sz9sqox5QEgrg?mode=hqctcla")],
        [bc("📲 BlazeNXT OTP",        url="https://t.me/+DfUnv_qgZEFkZWU8")],
...
def get_admin_keyboard(uid: int = 0):
    ...
    if row: rows.append(row)
    row = []
    if has_perm(uid, "fetch_sms"):   row.append(bc("🔄 Fetch SMS",   cb="menu_fetch"))
    if has_perm(uid, "otp_history"): row.append(bc("📋 OTP History", cb="menu_otp_history"))
    if row: rows.append(row)
    row = []
    if has_perm(uid, "groups"):    row.append(bc("👥 Groups",    cb="menu_groups"))
    if has_perm(uid, "broadcast"): row.append(bc("📢 Broadcast", cb="broadcast"))
    if row: rows.append(row)
    row = []
    if has_perm(uid, "settings"):  row.append(bc("⚙️ Settings",  cb="menu_settings"))
...
async def callback_query_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "confirm_add_panel":
        ...
        # Always store base URL + token separately — _fetch_ps_api combines them safely
        clean_url   = pd["url"].split("?")[0].rstrip("/")
        clean_token = pd["token"].strip()
        API_PANELS[pd["name"]] = {
            "url":      clean_url,
            "token":    clean_token,
        ...
        await query.edit_message_text(
            ...
            f"🟢 Worker is running!",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup([[b("📋 View Panels","list_panels")],[b("🔙 Back","menu_panels")]]))
        return
